...
export class ReportesTabComponent implements OnInit {
  ...
  seleccionarReporte(rep: ReporteCatalogoItem): void {
    if (!rep.disponible) {
      this.notificationService.info(`El reporte "${rep.titulo}" estará disponible en la próxima actualización.`);
      return;
    }
  ...
  descargarPdf(): void {
    ...
    if (lista.length === 0) {
      this.notificationService.info('No existen registros para exportar con los filtros seleccionados.');
      return;
    }
    ...
    try {
      ...
      const metadatos: MetadatosReporteInstitucional = {
        ...
        periodoAcademico: periodoObj ? `${periodoObj.nombre} (${periodoObj.idPeriodo})` : this.periodoSeleccionado() || 'Ciclo Actual',
      ...
      this.notificationService.success('Reporte oficial en PDF generado y descargado exitosamente.');
    } catch (err) {

...
export class AuthService {
  ...
  private readonly API_URL = 'http://localhost:5032/api/v1/auth';
  private readonly TOKEN_KEY = 'titan_access_token';
  private readonly REFRESH_TOKEN_KEY = 'titan_refresh_token';
  private readonly USER_INFO_KEY = 'titan_user_info';
  ...
  public hasPermission(moduleName: string, operationName: string): boolean {
    ...
    return modulo.operaciones.some((op) => op.toLowerCase() === operationName.toLowerCase());
  }

  public hasRole(roleCode: string): boolean {
    const user = this.currentUser();
    if (!user || !user.roles) return false;
    const cleanTarget = roleCode.toUpperCase().replace(/^TITAN_/, '');
    return user.roles.some((r) => {
      const upperRole = r.toUpperCase();
      const cleanRole = upperRole.replace(/^TITAN_/, '');
      return (

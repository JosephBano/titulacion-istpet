...
public class ActoresController : ControllerBase
{
    ...
    /// <summary>
    /// Buscar alumnos por cédula, nombres o apellidos
    /// </summary>
    [HttpGet("alumnos")]
    ...
    /// <summary>
    /// Obtener información de un estudiante por número de cédula
    /// </summary>
    ...
    public async Task<ActionResult<AlumnoResponseDto>> GetAlumnoPorCedula(string cedula, CancellationToken cancellationToken)
    {
        ...
        if (alumno == null)
        {
            return NotFound(new { message = $"El estudiante con cédula {cedula} no existe." });
        }
    ...
    /// <summary>
    /// Listar los docentes evaluadores activos en la institución
    /// </summary>
    [HttpGet("docentes")]
    ...
    /// <summary>
    /// Obtener información de un docente por número de cédula
    /// </summary>
    ...
    public async Task<ActionResult<ProfesorResponseDto>> GetDocentePorCedula(string cedula, CancellationToken cancellationToken)
    {
        ...
        if (docente == null)
        {
            return NotFound(new { message = $"El docente con cédula {cedula} no existe." });
        }
    ...
    /// </summary>
    [HttpGet("alumnos/{idAlumno}/matriculas")]
    public async Task<ActionResult<IEnumerable<MatriculaResponseDto>>> GetMatriculasPorAlumno(string idAlumno, CancellationToken cancellationToken)
    {
        var matriculas = await _actoresService.GetMatriculasPorAlumnoAsync(idAlumno, cancellationToken);
    ...
    /// <summary>
    /// Listar alumnos aptos para titulación
    /// </summary>

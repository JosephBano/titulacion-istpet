...
public static class RbacDbSeeder
{
    public static async Task SeedAsync(TitanDbContext context, IPasswordHasher passwordHasher)
    {
        ...
        var rolesDefiniciones = new Dictionary<string, string>
        {
            ...
            { "DOCENTE", "Docente Tutor y Evaluador de Tribunal" },
            { "ESTUDIANTE", "Estudiante Postulante de Titulación" },
            { "COORDINADOR", "Coordinador de Carrera y Titulación" },
        ...
        var adminRol = rolesEntities["ADMINISTRADOR"];
        foreach (var mo in modulosOpsEntities)
        {
            await AssignPermissionIfNotExistAsync(context, adminRol.idRol, mo.idModulosOperaciones);
        }

        // DOCENTE -> Consultar, Editar avances, Registrar Notas, Suscribir Actas
        var docenteRol = rolesEntities["DOCENTE"];
        ...
        // ESTUDIANTE -> Consultar, Crear postulación
        var estudianteRol = rolesEntities["ESTUDIANTE"];
